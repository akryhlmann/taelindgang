...
if [[ -n "$CAMERA_IP" ]]; then
    ...
    RTSP_GUESS="rtsp://brugernavn:adgangskode@${CAMERA_IP}:554/stream1"
    echo ""
    echo "Opdatér RTSP-URL i device1/config.yaml:"
    echo "    camera:"
    echo "      rtsp_url: \"${RTSP_GUESS}\""
    echo ""
    echo "TP-Link kamera RTSP-formater:"
    echo "  Tapo / Vigi (anbefalet):"
    echo "    rtsp://brugernavn:adgangskode@${CAMERA_IP}:554/stream1   (HD)"
    echo "    rtsp://brugernavn:adgangskode@${CAMERA_IP}:554/stream2   (SD, lavere CPU)"
    echo ""
    echo "  Credentials = Camera Account (IKKE Tapo-app-konto):"
    echo "    Tapo-app → Kamera → Indstillinger → Avanceret → Camera Account"
    echo ""
    echo "  Ældre TP-Link IPC-serie:"
    echo "    rtsp://admin:adgangskode@${CAMERA_IP}:554/h264/ch1/main/av_stream"
    echo ""
