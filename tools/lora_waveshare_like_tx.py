#!/usr/bin/env python3
"""
TX test that mirrors the Waveshare LoRaRF library init sequence
using lgpio instead of RPi.GPIO. This deliberately omits
SetRegulatorMode and Calibrate to match what the Waveshare demo does.

Run this on Device 1 while lora_waveshare_like_rx.py runs on Device 2.
"""
import time, sys

FREQUENCY  = 868_000_000
SF         = 7
BW         = 125_000
CS_PIN     = 21; RESET_PIN = 18; BUSY_PIN = 20; DIO1_PIN = 16; TXEN_PIN = 6
SPI_BUS    = 0;  SPI_DEVICE = 0

SYNC_WORD  = 0x3444   # Waveshare demo default (public LoRa network)

_BW_MAP = {125_000: 0x04, 250_000: 0x05, 500_000: 0x06}


def main():
    print("=== LoRa TX (Waveshare-like init) ===")
    print(f"Sending on 868 MHz SF{SF} BW{BW//1000}kHz sync=0x{SYNC_WORD:04X}")
    print("Press Ctrl+C to stop\n")

    try:
        import spidev, lgpio
    except ImportError as e:
        print(f"Missing: {e}"); sys.exit(1)

    lg = lgpio
    h = lg.gpiochip_open(0)
    lg.gpio_claim_output(h, CS_PIN,    1)
    lg.gpio_claim_output(h, RESET_PIN, 1)
    lg.gpio_claim_input(h,  BUSY_PIN)
    lg.gpio_claim_input(h,  DIO1_PIN)
    lg.gpio_claim_output(h, TXEN_PIN,  0)

    spi = spidev.SpiDev()
    spi.open(SPI_BUS, SPI_DEVICE)
    spi.max_speed_hz = 8_000_000
    spi.mode = 0

    def busy(timeout=2.0):
        t = time.time() + timeout
        while lg.gpio_read(h, BUSY_PIN):
            if time.time() > t: raise TimeoutError("BUSY")
            time.sleep(0.0001)

    def cmd(data):
        busy()
        lg.gpio_write(h, CS_PIN, 0)
        r = spi.xfer2(data)
        lg.gpio_write(h, CS_PIN, 1)
        return r

    def wreg(addr, val):
        cmd([0x0D, (addr>>8)&0xFF, addr&0xFF, val])

    def rreg(addr):
        r = cmd([0x1D, (addr>>8)&0xFF, addr&0xFF, 0x00, 0x00])
        return r[4]

    # --- Reset ---
    lg.gpio_write(h, RESET_PIN, 0); time.sleep(0.001)
    lg.gpio_write(h, RESET_PIN, 1); time.sleep(0.02)
    busy()

    # --- begin() equivalent ---
    # SetStandby RC
    cmd([0x80, 0x00])
    # SetPacketType LoRa
    cmd([0x01, 0x01])
    # _fixResistanceAntenna: OR reg 0x08D8 with 0x1E (Semtech errata)
    wreg(0x08D8, rreg(0x08D8) | 0x1E)

    # --- setDio2RfSwitch ---
    cmd([0x9D, 0x01])

    # --- setFrequency (includes calibrateImage) ---
    cmd([0x98, 0xD7, 0xDB])    # CalibrateImage 863-870 MHz
    freq = int(FREQUENCY / 32e6 * (1 << 25))
    cmd([0x86, (freq>>24)&0xFF, (freq>>16)&0xFF, (freq>>8)&0xFF, freq&0xFF])

    # --- setTxPower +22 dBm (Waveshare demo value) ---
    cmd([0x95, 0x04, 0x07, 0x00, 0x01])  # SetPaConfig +22dBm
    cmd([0x8E, 0x16, 0x05])              # SetTxParams 22dBm, 800us ramp

    # --- setLoRaModulation: SF7 BW125 CR4/5 LDRO off ---
    bw_idx = _BW_MAP[BW]
    cmd([0x8B, SF, bw_idx, 0x01, 0x00])

    # --- setLoRaPacket: explicit header, 12 preamble, CRC on, IQ standard ---
    cmd([0x8C, 0x00, 0x0C, 0x00, 0xFF, 0x01, 0x00])
    # _fixInvertedIq(False): ensure bit 2 of 0x0736 is clear
    wreg(0x0736, rreg(0x0736) & 0xFB)

    # --- setSyncWord ---
    wreg(0x0740, (SYNC_WORD >> 8) & 0xFF)
    wreg(0x0741, SYNC_WORD & 0xFF)

    # --- _fixLoRaBw500: set bit 2 of 0x0889 for BW < 500kHz (Semtech errata) ---
    # (done each TX, but pre-set here too)
    wreg(0x0889, rreg(0x0889) | 0x04)

    # --- SetBufferBase TX=0 RX=0 ---
    cmd([0x8F, 0x00, 0x00])

    # --- SetDioIrq: TX_DONE + TIMEOUT on DIO1 ---
    cmd([0x08, 0x02, 0x01, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00])

    print("Init OK — starting TX loop (sync=0x3444, +22dBm)\n")

    count = 0
    try:
        while True:
            count += 1
            payload = list(f"TEST_{count:04d}".encode())

            # clearIrq, writeBuffer, update payload length
            cmd([0x02, 0xFF, 0xFF])
            cmd([0x0E, 0x00] + payload)
            cmd([0x8C, 0x00, 0x0C, 0x00, len(payload), 0x01, 0x00])

            # _fixLoRaBw500 before each TX
            wreg(0x0889, rreg(0x0889) | 0x04)

            # TXEN=LOW for TX path on Waveshare module
            lg.gpio_write(h, TXEN_PIN, 0)
            cmd([0x83, 0x00, 0x00, 0x00])   # SetTx no timeout

            ok = False
            deadline = time.time() + 3.0
            while time.time() < deadline:
                r = cmd([0x12, 0x00, 0x00, 0x00])   # GetIrq
                irq = (r[2] << 8) | r[3]
                if irq & 0x0001:
                    ok = True
                    break
                time.sleep(0.005)

            ts = time.strftime("%H:%M:%S")
            status = "TX_DONE ✓" if ok else "TIMEOUT ✗"
            print(f"[{ts}] TX #{count}: 'TEST_{count:04d}' — {status}")

            time.sleep(5)

    except KeyboardInterrupt:
        print(f"\nStopped after {count} transmissions.")
    finally:
        cmd([0x80, 0x00])
        spi.close()
        lg.gpiochip_close(h)


if __name__ == "__main__":
    main()
